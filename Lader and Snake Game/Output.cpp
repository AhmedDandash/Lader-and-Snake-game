...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	y = (UI.CellHeight * cellPos.VCell()) + UI.ToolBarHeight;
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->SetPen(UI.CardNumColor); // order of shape
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	// we have to validate vertical cells first
	// eqn --->  toCell.HCell() == fromCell.VCell()
	// we have to validate that cell num of to cell is greater than that of from cell
	// eqn ---> toCell.getCellNum() > fromCEll.getCellNum()
	//// WE have to print an error message

	if ((toCell.HCell() == fromCell.HCell()) && (toCell.GetCellNum() > fromCell.GetCellNum()))
	{
		...
		int n = (toCell.GetCellNum() - fromCell.GetCellNum()) / 10;
		int j = fromStartY;

		for (int i = 0; i < n; i++)
		{

			pWind->DrawLine(x12, j, x34, j);
			j -= UI.CellHeight;
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if ((toCell.HCell() == fromCell.HCell()) && (toCell.GetCellNum() < fromCell.GetCellNum()))
	{
		...
		int sx[] = { sx1,sx2,sx3,sx4 };
		int sy[] = { sy1,sy2,sy3,sy4 };

		///TODO: Draw the Polygon (diamond) representing the Snake's Head
		//       Check the snakes drawn in the project document and draw it the same way
		//pWind->DrawPolygon(arrayofx, arrayofy, numoftheelements);

		pWind->DrawPolygon(sx, sy, 4);
	}
	else
	{
		PrintMessage("Error");
